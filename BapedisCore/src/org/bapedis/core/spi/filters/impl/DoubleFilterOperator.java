/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package org.bapedis.core.spi.filters.impl;

/**
 *
 * @author loge
 */
public enum DoubleFilterOperator implements FilterOperator {
    EQUALS("=") {

                @Override
                public boolean applyTo(Object obj, String operand, boolean matchCase) {
                    double val1 = (double) obj;
                    double val2 = Double.parseDouble(operand);
                    return val1 == val2;
                }
            },
    NOT_EQUALS("<>") {

                @Override
                public boolean applyTo(Object obj, String operand, boolean matchCase) {
                    double val1 = (double) obj;
                    double val2 = Double.parseDouble(operand);
                    return val1 != val2;
                }

            },
    LESS("<") {

                @Override
                public boolean applyTo(Object obj, String operand, boolean matchCase) {
                    double val1 = (double)obj;
                    double val2 = Double.parseDouble(operand);
                    return val1 < val2;
                }

            }, 
    LESS_OR_EQUALS("<=") {

                @Override
                public boolean applyTo(Object obj, String operand, boolean matchCase) {
                    double val1 = (double)obj;
                    double val2 = Double.parseDouble(operand);
                    return val1 <= val2;
                }

            }, 
    GREATER(">") {

                @Override
                public boolean applyTo(Object obj, String operand, boolean matchCase) {
                    double val1 = (double)obj;
                    double val2 = Double.parseDouble(operand);
                    return val1 > val2;
                }

            }, 
    GREATER_OR_EQUALS(">=") {

                @Override
                public boolean applyTo(Object obj, String operand, boolean matchCase) {
                    double val1 = (double)obj;
                    double val2 = Double.parseDouble(operand);
                    return val1 >= val2;
                }

            };
    private final String text;

    private DoubleFilterOperator(String text) {
        this.text = text;
    }

    @Override
    public boolean isValid(String operand) {
        try {
            Double.parseDouble(operand);
            return true;
        } catch (NumberFormatException ex) {
            return false;
        }
    }

    @Override
    public String toString() {
        return text; //To change body of generated methods, choose Tools | Templates.
    }
    
}
