...
@ActionID(
        category = "Edit",
        id = "org.bapedis.core.ui.actions.RemovePeptides"
)
@ActionRegistration(
        displayName = "RemovePeptides.name",
        lazy = false
)
@ActionReferences({
    @ActionReference(path = "Actions/EditPeptides", position = 20)
})
public class RemovePeptides extends GlobalContextSensitiveAction<Peptide> {
    ...
    public RemovePeptides() {
        super(Peptide.class);
        String name = NbBundle.getMessage(RemovePeptides.class, "RemovePeptides.name");
        putValue(NAME, name);
