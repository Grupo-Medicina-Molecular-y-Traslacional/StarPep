    ...
    if (configurationPtr > 0) {
      ...
      if (atom.alternateLocationID != '\0') {
        ...
        if (conformationIndex != 0) {
          Utilities.getIO().getOut().println("ignoring " + atom.atomName);
          return false;
        }
      }
    }
      ...
      for (int i = 0; i < tokenizer.fieldCount; ++i) {
        ...
        case GEOM_BOND_DISTANCE:
          ...
          if (pt >= 0) {
            ...
            if (Float.isNaN(dx)) {
              Utilities.getIO().getOut().println("error reading uncertainty for " + line);
              dx = 0.015f;
            }
...
*/
  ...
  private void setBondingAndMolecules() {
    Utilities.getIO().getOut().println("CIF creating molecule "
    ...
    if (isMolecular) {
      ...
      for (int i = firstAtom; i < atomCount; i++) {
        if (atomSetCollection.bsAtoms.get(i))
          ...
          Utilities.getIO().getOut().println(molecularType + " removing " + i + " "
