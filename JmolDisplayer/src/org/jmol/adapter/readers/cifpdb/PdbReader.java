 ...
 protected void initializeReader() throws Exception {
  ...
  protected void finalizeReader() throws Exception {
    ...
    if (configurationPtr > 0) {
      Utilities.getIO().getOut().println(sbSelected.toString());
      Utilities.getIO().getOut().println(sbIgnored.toString());
    }
  }
    ...
    while (true) {
      ...
      try {
        if (line.startsWith("REMARK 350 BIOMOLECULE:")) {
          if (nBiomt > 0)
            Utilities.getIO().getOut().println("biomolecule " + iMolecule + ": number of transforms: "
                + nBiomt);
        ...
        if (line.indexOf("APPLY THE FOLLOWING TO CHAINS:") >= 0) {
          ...
          if (checkFilter("BIOMOLECULE " + iMolecule + ";")) {
            ...
            Utilities.getIO().getOut().println("filter set to \"" + filter + "\"");
            this.biomts = biomts;
          }
