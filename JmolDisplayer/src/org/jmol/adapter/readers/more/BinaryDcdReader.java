...
ICNTRL(20)=VERNUM ! version number 
  ...
  protected void readDocument() throws Exception {
    ...
    n = doc.readInt(); // TRAILER
    Utilities.getIO().getOut().println("BinaryDcdReadaer:\n" + sb);
    ...
    if (nFixed != 0) {
      ...
      Utilities.getIO().getOut().println("free: " + bsFree.cardinality() + " " + Escape.escape(bsFree));
    }
    
    readCoordinates();
    
    Utilities.getIO().getOut().println("Total number of trajectory steps=" + trajectorySteps.size());
  }
