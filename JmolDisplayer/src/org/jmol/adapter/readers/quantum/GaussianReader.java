...
public class GaussianReader extends MOReader {
  ...
  protected boolean checkLine() throws Exception {
    ...
    if (line.indexOf("Input orientation:") >= 0
      ...
      equivalentAtomSets++;
      //if (Logger.debugging)
        Utilities.getIO().getOut().println(atomSetCollection.getAtomSetCount() + " model " + modelNumber + " step " + stepNumber
    ...
    if (line.indexOf("Molecular Orbital Coefficients") >= 0) {
      ...
      Utilities.getIO().getOut().println(orbitals.size() + " molecular orbitals read");
      return true;
    }
...
//  atomSetCollection.setAtomSetName("Last read atomset.");
  ...
  private void readBasis() throws Exception {
    ...
    Utilities.getIO().getOut().println(shellCount + " slater shells read");
    Utilities.getIO().getOut().println(gaussianCount + " gaussian primitives read");
  }
...
but:
 ...
 105        4S        -47.27845  63.29565-100.44035   1.98362 -51.35328
  ...
  void readDipoleMoment() throws Exception {
    ...
    Utilities.getIO().getOut().println("Molecular dipole for model " + atomSetCollection.getAtomSetCount()
        + " = " + dipole);
  ...
  void readPartialCharges() throws Exception {
    ...
    Utilities.getIO().getOut().println("Mulliken charges found for Model " + atomSetCollection.getAtomSetCount());
  }
