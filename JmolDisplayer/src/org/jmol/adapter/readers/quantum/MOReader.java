...
abstract public class MOReader extends BasisFunctionReader {
  ...
  private void getNboCharges() throws Exception {
    ...
    for (int i = i0; i < atomCount; ++i) {
      ...
      if (tokens == null || tokens.length < 3 || Float.isNaN(charge = parseFloat(tokens[2]))) {
        Utilities.getIO().getOut().println("Error reading NBO charges: " + line);
        return;
      }
    ...
    Utilities.getIO().getOut().println("Using NBO charges for Model " + atomSetCollection.getAtomSetCount());
  }
 ...
 -------------------------------------------------------------------------------
  ...
  protected void getNboTypes() throws Exception {
    ...
    Utilities.getIO().getOut().println(n + " natural bond AO basis functions found");
  }
