...
public class MoldenReader extends MopacSlaterReader {
  ...
  private boolean readGaussianBasis() throws Exception {
    ...
    Utilities.getIO().getOut().println(shells.size() + " slater shells read");
    Utilities.getIO().getOut().println(garray.length + " gaussian primitives read");
    Utilities.getIO().getOut().println(nCoef + " MO coefficients expected for orbital type " + orbitalType);
