...
public class CrystalReader extends AtomSetCollectionReader {
  ...
  private boolean setDirect() throws Exception {
    ...
    .getMatrix();
    Utilities.getIO().getOut().println("oriented unit cell is in model "
  ...
  throws Exception {
    ...
    atomSetCollection.setAtomSetAtomProperty(name, data);
  }
  ...
  private boolean readPrimitiveMapping() throws Exception {
    ...
    Utilities.getIO().getOut().println(nPrim + " primitive atoms and " + atomCount
        + " conventionalAtoms");
