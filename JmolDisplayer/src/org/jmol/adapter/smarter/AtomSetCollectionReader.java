 ...
 * setUnitCellItem() setAtomCoord()
 *
 * At the very minimum, you need:
 *
 * setAtomCoord()
 *
 * so that: (a) atom coordinates can be turned fractional by load parameters (b)
 * symmetry can be applied once per model in the file
 *
 * If you know where the end of the atom+bond data are, then you can use
 * applySymmetryAndSetTrajectory() once, just before exiting. Otherwise, use it
 * twice -- it has a check to make sure it doesn't RUN twice -- once at the
 * beginning and once at the end of the model.
 *
 * htParams is used for passing information to the readers and for returning
 ...
 * from Eval or Viewer:
 *
 * applySymmetryToBonds atomTypes (for Mol2Reader) bsModels filter firstLastStep
 * firstLastSteps getHeader isTrajectory lattice manifest (for
 * SmarterJmolAdapter) modelNumber spaceGroupIndex symmetryRange unitcell packed
 *
 * from FileManager:
 *
 * fullPathName subFileList (for SmarterJmolAdapter)
 *
 * from MdTopReader:
 *
 * isPeriodic templateAtomCount
 *
 * from MdCrdReader:
 *
 * trajectorySteps
 ...
 * from Resolver:
 *
 * filteredAtomCount ptFile readerName templateAtomCount
 *
 *
...
public abstract class AtomSetCollectionReader {
    ...
    /*
     * public void finalize() { System.out.println(this + " finalized"); }
     */
...
//        io = IOProvider.getDefault().getIO("JMol Output", false);
//        io.select();
//        Utilities.getIO().getOut().println("Hello from standard 0 out");
//        Utilities.getIO().getOut().println("Hello from standard 1 out");
//        io.getErr().println("Hello from standard err");  //this text should appear in red
//        Utilities.getIO().close();
//        io.getErr().close();
    }
    ...
    private Object finish() {
        ...
        if (htParams.containsKey("bsFilter")) {
            htParams.put("filteredAtomCount", Integer.valueOf(BitSetUtil.cardinalityOf((BitSet) htParams.get("bsFilter"))));
        }
    ...
    private void initialize() {
        ...
        int ptFile = (htParams.containsKey("ptFile") ? ((Integer) htParams.get("ptFile")).intValue() : -1);
        ...
        if (htParams.containsKey("templateAtomCount")) {
            templateAtomCount = ((Integer) htParams.get("templateAtomCount")).intValue();
        }
        ...
        symmetryRange = (htParams.containsKey("symmetryRange") ? ((Float) htParams.get("symmetryRange")).floatValue() : 0);
        ...
        if (htParams.containsKey("spaceGroupIndex")) {
            ...
            desiredSpaceGroupIndex = ((Integer) htParams.get("spaceGroupIndex")).intValue();
    ...
    protected void newAtomSet(String name) {
        ...
        Logger.debug(name);
        Utilities.getIO().getOut().println(name);
    }
    ...
    public void setSpaceGroupName(String name) {
        ...
        Utilities.getIO().getOut().println("Setting space group name to " + spaceGroup);
        Utilities.getIO().getOut().println("Setting space group name to " + spaceGroup);
    }
    ...
    public void setSymmetryOperator(String xyz) {
        ...
        if (!atomSetCollection.addSpaceGroupOperation(xyz)) {
            ...
            Utilities.getIO().getOut().println("Skipping symmetry operation " + xyz);
        }
    ...
    public void setUnitCellItem(int i, float x) {
        ...
        if (Logger.debugging) {
            ...
            Utilities.getIO().getOut().println("setunitcellitem " + i + " " + x);
        }
    ...
    protected void setFilter(String filter0) {
        ...
        if (bsFilter == null) {
            ...
            Utilities.getIO().getOut().println("filtering with " + filter);
            Utilities.getIO().getOut().println("filtering with " + filter);
            if (haveAtomFilter) {
                int ipt;
                filter1 = filter;
    ...
    public void setTransform(float x1, float y1, float z1, float x2, float y2,
        ...
        Utilities.getIO().getOut().println("defaultOrientationMatrix = " + matrixRotate);
        Utilities.getIO().getOut().println("defaultOrientationMatrix = " + matrixRotate);
    ...
    public void setMOData(Map<String, Object> moData) {
        ...
        if (orbitals != null) {
            Utilities.getIO().getOut().println(orbitals.size() + " molecular orbitals read in model "
     ...
     * @param ignore the frequencies to ignore because the user has selected
     * only certain vibrations to be read or for whatever reason; length serves
     * to set the number of frequencies to be read
     * @param isWide when TRUE, this is a table that has X Y Z for each mode
     * within the same row; when FALSE, this is a table that has X Y Z for each
     * mode on a separate line.
     * @param col0 the column in which data starts
     * @param colWidth the width of the data columns
     * @param atomIndexes an array either null or indicating exactly which atoms
     * get the frequencies (used by CrystalReader)
     * @throws Exception
     */
            ...
            for (int j = 0, jj = 0; jj < nFreq; jj++) {
                ...
                if (Logger.debugging) {
                    Logger.debug("atom " + iAtom + " vib" + j + ": " + vx + " " + vy + " "
                            + vz);
                    Utilities.getIO().getOut().println("atom " + iAtom + " vib" + j + ": " + vx + " " + vy + " "
                            + vz);
                }
    ...
    public void checkLineForScript() {
        if (line.indexOf("Jmol") >= 0) {
            ...
            if (line.indexOf("Jmol data min") >= 0) {
                ...
                parseStringInfestedFloatArray(line.substring(10).replace('=', ' ').replace('{', ' ').replace('}', ' '), data);
                 ...
                 * System.out.println(unitCellOffset); unitCellOffset = new
                 * Point3f(maxXYZ); symmetry.toCartesian(unitCellOffset);
                ...
                /*
                 * Point3f pt = new Point3f(minXYZ); symmetry.toCartesian(pt);
                 * System.out.println("ASCR minXYZ " + pt); pt.set(maxXYZ);
                 * symmetry.toCartesian(pt); System.out.println("ASCR maxXYZ " +
                 * pt);
                 */
    ...
    protected void addJmolScript(String script) {
        Utilities.getIO().getOut().println("#jmolScript: " + script);
        Utilities.getIO().getOut().println("#jmolScript: " + script);
    ...
    /*
     * static { System.out.println(Escape.toJSON(null,
     * getFortranFormatLengths("(5A10")));
     * System.out.println(Escape.toJSON(null,
     * getFortranFormatLengths("(5E10.3,2A4.6")));
     * System.out.println(Escape.toJSON(null,
     * getFortranFormatLengths("(5A4,2X,2I20"))); }
     */
