/* $RCSfile$
 * $Author: hansonr $
 * $Date: 2011-09-29 04:11:08 +0200 (jeu., 29 sept. 2011) $
 * $Revision: 16177 $
 *
  ...
  static Object getAtomCollectionReader(String fullName, String type,
    ...
    if (type != null) {
      ...
      if (readerName == null)
        ...
        Utilities.getIO().getOut().println("The Resolver assumes " + readerName);
    } else {
      ...
      if (readerName.charAt(0) == '\n')
        ...
        Utilities.getIO().getOut().println("The Resolver thinks " + readerName);
    }
    if (errMsg != null) {
  ...
  private void dumpLines(String[] lines) {
      for (int i = 0; i < lines.length; i++) {
        ...
        for (int j = 0; j < lines[i].length(); j++)
          Utilities.getIO().getOut().println("\t"+(int)lines[i].charAt(j));
      }
      Utilities.getIO().getOut().println("");
  }
