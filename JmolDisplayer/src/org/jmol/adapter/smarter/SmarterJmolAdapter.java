/* $RCSfile$
 * $Author: hansonr $
 * $Date: 2011-09-14 13:32:25 +0200 (mer., 14 sept. 2011) $
 * $Revision: 16068 $
 *
...
public class SmarterJmolAdapter extends JmolAdapter {
  ...
  private static Object staticGetAtomSetCollectionOrBufferedReaderFromZip(
    ...
    if (haveManifest) {
      if (Logger.debugging)
        Utilities.getIO().getOut().println("manifest for  " + fileName + ":\n" + manifest);
      ...
      if (haveManifest && !exceptFiles) {
        ...
        for (int i = 0; i < list.length; i++) {
          ...
          if (htCollections.containsKey(file))
            ...
            Utilities.getIO().getOut().println("manifested file " + file + " was not found in "
                + fileName);
        }
      }
      if (!doCombine)
