...
<efvet xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  void getSurfaceData() throws Exception{
    ...
    getVertices();
    getTriangles();
    Utilities.getIO().getOut().println("efvet file contains " + nVertices + " vertices and " + nTriangles + " triangles");
  }
  ...
  private void getHeader() throws Exception {
    ...
    Utilities.getIO().getOut().println(jvxlFileHeaderBuffer.toString());
  }
  ...
  private void getVertices() throws Exception {
    ...
    if (dataIndex > 0 && dataIndex < tokens.length)
      Utilities.getIO().getOut().println("property " + tokens[dataIndex]);
    else
