...
class IsoSolventReader extends AtomDataReader {
  ...
  private void generateSolventCavity() {
    ...
    Utilities.getIO().getOut().println("cavities include " + n + " voxel points");
  ...
  private void generateSolventCube() {
    ...
    if (doCalculateTroughs) {
      ...
      getEdges();
      Utilities.getIO().getOut().println(vEdges.size() + " edges");
      ...
      getFaces();
      Utilities.getIO().getOut().println(vFaces.size() + " faces");
      vEdges = null;
      bsLocale = null;
      htEdges = null;
 ...
 */
  ...
  public float getValueAtPoint(Point3f pt) {
    ...
    //does not work: float value = (jvxlData.jvxlPlane != null ? 0.001f : Float.MAX_VALUE);
