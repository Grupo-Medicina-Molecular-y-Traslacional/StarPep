...
public class JvxlXmlReader extends VolumeFileReader {
  ...
  protected void readParameters() throws Exception {
   ...
   Utilities.getIO().getOut().println("jvxl file surfaces: " + nSurfaces);
   Utilities.getIO().getOut().println("using default edge fraction base and range");
   Utilities.getIO().getOut().println("using default color fraction base and range");
  ...
  protected void gotoData(int n, int nPoints) throws Exception {
    if (n > 0)
      Utilities.getIO().getOut().println("skipping " + n + " data sets, " + nPoints + " points each");
  ...
  protected void jvxlReadSurfaceInfo() throws Exception {
    ...
    if (!Float.isNaN(jvxlCutoff))
      Utilities.getIO().getOut().println("JVXL read: cutoff " + jvxlCutoff);
    ...
    if (params.isContoured) {
      ...
      if (nContoursRead <= 0) {
        ...
        if (s.length() > 0) {
          ...
          Utilities.getIO().getOut().println("JVXL read: contourValues " + Escape.escapeArray(jvxlData.contourValues));            
        }
        ...
        if (s.length() > 0) {
          ...
          Utilities.getIO().getOut().println("JVXL read: contourColixes " +
    ...
    if (s.indexOf("{") >= 0) {
      try {
        ...
        Utilities.getIO().getOut().println("JVXL read: plane " + params.thePlane);
        if (params.scale3d == 0)
  ...
  protected void jvxlSetColorRanges(float dataMin, float dataMax, float red,
    ...
    if (!Float.isNaN(dataMin) && !Float.isNaN(dataMax)) {
      ...
      Utilities.getIO().getOut().println("JVXL read: data_min/max " + params.mappedDataMin + "/"
    ...
    Utilities.getIO().getOut().println("JVXL read: color red/blue: " + params.valueMappedToRed + "/"
  ...
  protected String readColorData() {
    ...
    */
    ...
    Utilities.getIO().getOut().println("JVXL reading color data mapped min/max: "
    ...
    Utilities.getIO().getOut().println("mapping red-->blue for " + params.valueMappedToRed + " to "
  ...
  protected void getEncodedVertexData() throws Exception {
    ...
    Utilities.getIO().getOut().println("Checking for vertex values");
  ...
  public Point3f[] jvxlDecodeVertexData(String data, boolean asArray) throws Exception {
    ...
    if (!asArray)
      Utilities.getIO().getOut().println("Reading " + vertexCount + " vertices");
  ...
  int[][] jvxlDecodeTriangleData(String data, String edgeData, String colorData)
    ...
    if (nData < 0)
      return null;
    Utilities.getIO().getOut().println("Reading " + nData + " triangles");
    int[][] triangles = null;
  ...
  protected void jvxlDecodeContourData(JvxlData jvxlData, String data) throws Exception {
    ...
    if (n > 0) {
      ...
      jvxlData.contourColors = Graphics3D.getHexCodes(jvxlData.contourColixes);
      Utilities.getIO().getOut().println("JVXL read: " + n + " discrete contours");
      Utilities.getIO().getOut().println("JVXL read: contour values: " + values);
      Utilities.getIO().getOut().println("JVXL read: contour colors: " + colors);
    }
  }
