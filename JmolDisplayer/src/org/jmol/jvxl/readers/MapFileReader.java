...
abstract class MapFileReader extends VolumeFileReader {
  ...
  protected void getVectorsAndOrigin() {
      ...
      Utilities.getIO().getOut().println("grid parameters: nx,ny,nz: " + nx + "," + ny + "," + nz);
    Utilities.getIO().getOut().println("grid parameters: nxStart,nyStart,nzStart: " + nxyzStart[0]
        + "," + nxyzStart[1] + "," + nxyzStart[2]);

    Utilities.getIO().getOut().println("grid parameters: mx,my,mz: " + na + "," + nb + "," + nc);
    Utilities.getIO().getOut().println("grid parameters: a,b,c,alpha,beta,gamma: " + a + "," + b + ","
        + c + "," + alpha + "," + beta + "," + gamma);
    Utilities.getIO().getOut().println("grid parameters: mapc,mapr,maps: " + mapc + "," + mapr + ","
        + maps);
    Utilities.getIO().getOut().println("grid parameters: originX,Y,Z: " + origin);
    ...
    */               
    ...
    Utilities.getIO().getOut().println("Jmol unit cell vectors:");
    Utilities.getIO().getOut().println("    a: " + vectors[0]);
    Utilities.getIO().getOut().println("    b: " + vectors[1]);
    Utilities.getIO().getOut().println("    c: " + vectors[2]);
    ...
    Utilities.getIO().getOut().println("Jmol grid origin in Cartesian coordinates: " + origin);
    Utilities.getIO().getOut().println("Use  isosurface OFFSET {x y z}  if you want to shift it.\n");
...
Jmol origin in slow-to-fast system: (19.333334, -6.9282017, -21.666666)
  ...
  protected void setCutoffAutomatic() {
    if (params.thePlane == null && params.cutoffAutomatic) {
      ...
      Utilities.getIO().getOut().println("DNS6Reader: setting cutoff to default value of "
          + params.cutoff
