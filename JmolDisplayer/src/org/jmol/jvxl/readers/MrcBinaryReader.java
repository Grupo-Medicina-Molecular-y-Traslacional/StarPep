  ...
  protected void readParameters() throws Exception {
    ...
    if (nx < 0 || nx > 1<<8) {
      ...
      if (params.thePlane == null)
        params.insideOut = !params.insideOut;
      if (nx < 0 || nx > 1000) {
        Utilities.getIO().getOut().println("nx=" + nx + " not displayable as MRC file");
        throw new Exception("MRC file type not readable");
      }
      Utilities.getIO().getOut().println("reading nonstandard little-endian MRC file");
    }
    ...
    Utilities.getIO().getOut().println("MRC header: mode: " + mode);
    ...
    Utilities.getIO().getOut().println("MRC header: dmin,dmax,dmean: " + dmin + "," + dmax + ","
        + dmean);

    ispg = binarydoc.readInt();
    nsymbt = binarydoc.readInt();

    Utilities.getIO().getOut().println("MRC header: ispg,nsymbt: " + ispg + "," + nsymbt);
    ...
    Utilities.getIO().getOut().println("MRC header: rms: " + rmsDeviation);

    nlabel = binarydoc.readInt();

    Utilities.getIO().getOut().println("MRC header: labels: " + nlabel);
    ...
    for (int i = 0; i < 10; i++) {
      ...
      if (i < nlabel) {
        labels[i] = s;
        Utilities.getIO().getOut().println(labels[i]);
      }
    }
    ...
    for (int i = 0; i < nsymbt; i++) {
      ...
      Utilities.getIO().getOut().println("MRC file symmetry information: " + s);
    }

    Utilities.getIO().getOut().println("MRC header: bytes read: " + binarydoc.getPosition()
        + "\n");
    ...
    if (params.thePlane == null && (params.cutoffAutomatic || !Float.isNaN(params.sigma))) {
      ...
      Utilities.getIO().getOut().println("Cutoff set to (mean + rmsDeviation*" + sigma + " = " + params.cutoff + ")\n");
    }
