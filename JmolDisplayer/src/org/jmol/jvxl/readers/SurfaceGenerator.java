...
*/  
  ...
  public boolean setParameter(String propertyName, Object value, BitSet bs) {
      ...
      if (value == null) {
        ...
        for (int i = 0; i < params.title.length; i++)
          if (params.title[i].length() > 0)
            Utilities.getIO().getOut().println(params.title[i]);
      }
      return true;
    }
    ...
    if ("radius" == propertyName) {
      Utilities.getIO().getOut().println("solvent probe radius set to " + value);
    ...
    if ("molecular" == propertyName || "solvent" == propertyName
      ...
      Utilities.getIO().getOut().println(params.calculationType);
      processState();
      return true;
    }
  ...
  private boolean setReader() {
    ...
    Utilities.getIO().getOut().println("Using surface reader " + surfaceReader);
    return true;
  }
  ...
  private SurfaceReader setFileData(Object value) {
    ...
    if (fileType == null)
      fileType = "UNKNOWN";
    Utilities.getIO().getOut().println("data file type was determined to be " + fileType);
