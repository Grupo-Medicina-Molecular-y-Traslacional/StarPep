...
abstract class VolumeFileReader extends SurfaceFileReader {
  ...
  protected void closeReader() {
    ...
    if (nData == 0 || dataMax == -Float.MAX_VALUE)
      return;
    dataMean /= nData;
    Utilities.getIO().getOut().println("VolumeFileReader closing file: " + nData
  ...
  protected boolean readVolumeData(boolean isMapData) {
    if (!gotoAndReadVoxelData(isMapData))
      return false;
    if (!vertexDataOnly)
      Utilities.getIO().getOut().println("JVXL read: " + nPointsX + " x " + nPointsY + " x "
  ...
  private int readVolumetricHeader() {
    try {
      ...
      if (downsampling) {
        ...
        Utilities.getIO().getOut().println("downsample factor = " + downsampleFactor);
        for (int i = 0; i < 3; ++i) {
          int n = voxelCounts[i];
          downsampleRemainders[i] = n % downsampleFactor;
          voxelCounts[i] /= downsampleFactor;
          volumetricVectors[i].scale(downsampleFactor);
          Utilities.getIO().getOut().println("downsampling axis " + (i + 1) + " from " + n + " to "
              + voxelCounts[i]);
        }
      }
      for (int i = 0; i < 3; ++i) {
        ...
        Utilities.getIO().getOut().println("voxel grid count/vector:" + line);
        if (!isAngstroms)
