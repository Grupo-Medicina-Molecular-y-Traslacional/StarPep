...
public class Minimizer implements MinimizerInterface {
  ...
  public boolean minimize(int steps, double crit, BitSet bsSelected,
    ...
    Utilities.getIO().getOut().println("minimize: initializing (steps = " + steps + " criterion = "
        + crit + ") ...");
  ...
  private boolean setupMinimization() {
    ...
    Utilities.getIO().getOut().println(GT._("{0} atoms will be minimized.", "" + atomCount));
    Utilities.getIO().getOut().println("minimize: creating bonds...");
    ...
    Utilities.getIO().getOut().println("minimize: setting atom types...");
    ...
    Utilities.getIO().getOut().println("minimize: getting angles...");
    getAngles();
    Utilities.getIO().getOut().println("minimize: getting torsions...");
    getTorsions();
...
*/        }
    ...
    Utilities.getIO().getOut().println(angles.length + " angles");
  }
...
*/          }
    ...
    Utilities.getIO().getOut().println(torsions.length + " torsions");
  ...
  public ForceField getForceField() {
    if (pFF == null) {
      try {
        ...
        Utilities.getIO().getOut().println( "minimize: using " + className);
        pFF = (ForceField) Class.forName(className).newInstance();
      } catch (Exception e) {
        Logger.error(e.getMessage());
      }
    }
    //Utilities.getIO().getOut().println("minimize: forcefield = " + pFF);
    return pFF;
  }
  ...
  public boolean startMinimization() {
   try {
      Utilities.getIO().getOut().println("minimizer: startMinimization");
  ...
  void endMinimization() {
    ...
    Utilities.getIO().getOut().println("minimizer: endMinimization");
}
