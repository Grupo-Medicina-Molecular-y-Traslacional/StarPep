...
*/
  ...
  private void createModelSet(JmolAdapter adapter, Object atomSetCollection,
    ...
    if (nAtoms > 0)
      Utilities.getIO().getOut().println("reading " + nAtoms + " atoms");
    ...
    if (adapter == null) {
      ...
      if (adapterModelCount > 0) {
        Utilities.getIO().getOut().println("ModelSet: haveSymmetry:" + modelSet.someModelsHaveSymmetry
            + " haveUnitcells:" + someModelsHaveUnitcells
            + " haveFractionalCoord:" + modelSet.someModelsHaveFractionalCoordinates);
        Utilities.getIO().getOut().println(adapterModelCount + " model" + (modelSet.modelCount == 1 ? "" : "s")
      ...
      if (q != null) {
        Utilities.getIO().getOut().println("defaultOrientationQuaternion = " + q);
        Logger
  ...
  private void setAtomProperties() {
    ...
    for (int i = baseModelIndex; i < modelCount; atomIndex += modelAtomCount, i++) {
      ...
      for (Map.Entry<String, String> entry : atomProperties.entrySet()) {
        ...
        Utilities.getIO().getOut().println("creating " + key + " for model " + modelSet.getModelName(i));
  ...
  private void iterateOverAllNewModels(JmolAdapter adapter, Object atomSetCollection) {
    ...
    if (isTrajectory) {
      ...
      Utilities.getIO().getOut().println(n + " trajectory steps read");
  ...
  private void finalizeModels(int baseModelCount) {
    ...
    for (int i = 0; i < modelCount; i++) {
      ...
      if (codes != null) {
        Utilities.getIO().getOut().println("model " + modelSet.getModelNumberDotted(i)
    ...
    for (int i = 0; i < modelSet.atomCount; i++) {
      if (atoms[i].modelIndex != iLast) {
        ...
        if (vdwtype != vdwtypeLast) {
          Utilities.getIO().getOut().println("Default Van der Waals type for model" + " set to " + vdwtype.getVdwLabel());
          vdwtypeLast = vdwtype;
        }
      }
    }
    Utilities.getIO().getOut().println(nRead + " atoms created");    
  }
  ...
  private void iterateOverAllNewBonds(JmolAdapter adapter, Object atomSetCollection) {
    ...
    if (haveMultipleBonds && modelSet.someModelsHaveSymmetry && !viewer.getApplySymmetryToBonds())
      Utilities.getIO().getOut().println("ModelSet: use \"set appletSymmetryToBonds TRUE \" to apply the file-based multiple bonds to symmetry-generated atoms.");
  ...
  private void freeze() {
    ...
    if (ret.length() > 0)
      Utilities.getIO().getOut().println(ret);
  }
