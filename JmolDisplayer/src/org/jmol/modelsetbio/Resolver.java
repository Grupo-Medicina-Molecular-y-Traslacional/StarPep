...
public final class Resolver implements JmolBioResolver {
  ...
  private String[][] getLigandBondInfo(JmolAdapter adapter, Object model, String group3) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (t != null)
        Utilities.getIO().getOut().println(" ligand " + group3 + ": " + bondInfo[i][0] + " - " + bondInfo[i][1] + " order " + bondInfo[i][2]);
    }
    return bondInfo;
  }
    ...
    if (bsAddedHydrogens.nextSetBit(0) >= 0) {
      ...
      for (int i = 0; i < pts.length; i++) {
        ...
        if (isChiral && n == 3 || isMethyl != (n == 3)) {
          Utilities.getIO().getOut().println("Error adding H atoms to " + gName + g.getResno() + ": "
...
//          Measure.getPlaneThroughPoints(pts[i][0], pts[i][1], pts[i][2], vNorm, vAB,
  ...
  private void finalizePdbMultipleBonds() {
    ...
    for (String key : htBondMap.keySet()) {
      ...
      Utilities.getIO().getOut().println("bond " + key + " was not used; order=" + value);
    ...
    for (int i = 0; i < bondCount; i++) {
      ...
      Utilities.getIO().getOut().println("assigning order " + bonds[i].order + " to bond " + bonds[i]);
    }
  }
