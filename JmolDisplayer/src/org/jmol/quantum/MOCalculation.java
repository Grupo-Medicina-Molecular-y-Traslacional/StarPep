...
public class MOCalculation extends QuantumCalculation implements
  ...
  public boolean setupCalculation(VolumeDataInterface volumeData, BitSet bsSelected,
    ...
    Utilities.getIO().getOut().println("Normalizing AOs: " + doNormalize + " slaters:" + (slaters != null));
  ...
  protected void process() {
    ...
    if (slaters == null) {
      ...
      //Utilities.getIO().getOut().println("Processing " + nShells + " Gaussian  shells");
      if (c < 0) {
        c = 0;
  ...
  private boolean checkCalculationType() {
    ...
    if (nwChemMode)
      Utilities.getIO().getOut().println("Normalization of contractions (NWCHEM)");
    ...
    if (calculationType.indexOf("?") >= 0) {
      ...
      Utilities.getIO().getOut().println("calculation type: " + calculationType + " OK.");
    }
    return true;
  }
...
*/ 
  ...
  public void calculateElectronDensity() {
    ...
    integration *= volume;
    Utilities.getIO().getOut().println("Integrated density = " + integration);
  }
