...
public class NciCalculation extends QuantumCalculation implements
  ...
  public boolean setupCalculation(VolumeDataInterface volumeData,
    ...
    if (parameters != null)
      Utilities.getIO().getOut().println("NCI calculation parameters = " + Escape.escape(parameters));
    ...
    Utilities.getIO().getOut().println("NCI calculation type = "
    ...
    if (qmAtoms != null) {
      ...
      for (int i = qmAtoms.length; --i >= 0;) {
        ...
        if (qmAtoms[i].znuc < 1) {
          ...
          Utilities.getIO().getOut().println("NCI calculation just setting nuclear charge for "
      ...
      if (type != TYPE_ALL) {
        for (int i = 0; i < bsMolecules.length; i++) {
          ...
          nMolecules++;
          Utilities.getIO().getOut().println("Molecule " + (nMolecules) + " ("
  ...
  private static double getParameter(float[] parameters, int i, double def, String name) {
    ...
    if (param == 0)
      param = def;
    Utilities.getIO().getOut().println("NCI calculation parameters[" + i + "] (" + name + ") = " + param);
    return param;
  }
  ...
  private void getBsOK() {
    ...
    Utilities.getIO().getOut().println("NCI calculation SCF " + (type == TYPE_INTRA ? "intra" : "inter") + "molecular grid points = " + bsOK.cardinality());
  }
