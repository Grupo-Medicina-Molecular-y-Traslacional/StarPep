...
public class ScriptEvaluator {
  ...
  private void pushContext(ContextToken token) throws ScriptException {
    ...
    if (debugScript || isCmdLine_c_or_C_Option)
      Utilities.getIO().getOut().println("-->>-------------".substring(0, Math
  ...
  private Point4f hklParameter(int i) throws ScriptException {
    ...
    if (!isSyntaxCheck && Logger.debugging)
      Utilities.getIO().getOut().println("defined plane: " + p);
    return p;
  }
  ...
  private boolean checkContinue() {
    ...
    if (Logger.debugging) {
      Utilities.getIO().getOut().println("script execution paused at command " + (pc + 1) + " level "
  ...
  private void instructionDispatchLoop(boolean doList) throws ScriptException {
    ...
    for (; pc < aatoken.length && pc < pcEnd; pc++) {
      ...
      if (!setStatement(pc)) {
        Utilities.getIO().getOut().println(getCommand(pc, true, false)
      ...
      if (isSyntaxCheck) {
        if (isCmdLine_c_or_C_Option)
          Utilities.getIO().getOut().println(thisCommand);
  ...
  float getSmilesCorrelation(BitSet bsA, BitSet bsB, String smiles,
    ...
    try {
      ...
      for (int i = 0; i < maps.length; i++) {
        ...
        Utilities.getIO().getOut().println("getSmilesCorrelation stddev=" + stddev);
        if (vReturn != null) {
          if (stddev < tolerance) {
            BitSet bs = new BitSet();
            for (int j = 0; j < maps[i].length; j++)
  ...
  private void mapProperty() throws ScriptException {
    ...
    while (true) {
      ...
      if (bsOut == null) {
        ...
        if (Logger.debugging)
          Utilities.getIO().getOut().println(sb.toString());
  ...
  private void load() throws ScriptException {
    ...
    if (os != null)
      try {
        ...
        Utilities.getIO().getOut().println(GT._("file {0} created", localName));
  ...
  private String plot(Token[] args) throws ScriptException {
    ...
    if (tok == Token.property) {
      ...
      Utilities.getIO().getOut().println("plot min/max: " + minXYZ + " " + maxXYZ);
    ...
    if (Logger.debugging)
      Utilities.getIO().getOut().println(data);
  ...
  */
  ...
  private RadiusData encodeRadiusParameter(int index, boolean isOnly,
    ...
    case Token.vanderwaals:
      ...
      vdwType = (tok == Token.vanderwaals ? null : EnumVdw.getVdwType2(Token
          .nameOf(tok)));
      tok = tokAt(++index);
      break;
    }
  ...
  private void setMoData(List<Object[]> propertyList, int moNumber,
    ...
    if (linearCombination == null) {
      ...
      if (offset != Integer.MAX_VALUE) {
        ...
        Utilities.getIO().getOut().println("MO " + moNumber);
      }
      ...
      case Token.string:
        ...
        if (isInline) {
          ...
          if (!isSyntaxCheck) {
            ...
            Utilities.getIO().getOut().println("reading isosurface data from " + filename);
