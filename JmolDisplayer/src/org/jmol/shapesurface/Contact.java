...
public class Contact extends Isosurface {
  ...
  private void setContacts(Object[] value) {
    ...
    float ptSize = (colorDensity && parameters != null && parameters[0] < 0 ? Math
        ...
        : 0.15f);
    if (Logger.debugging) {
      Utilities.getIO().getOut().println("Contact intramolecularMode " + intramolecularMode);
      Utilities.getIO().getOut().println("Contacts for " + bsA.cardinality() + ": "
          + Escape.escape(bsA));
      Utilities.getIO().getOut().println("Contacts to " + bsB.cardinality() + ": "
          + Escape.escape(bsB));
    }
  ...
  private float combineSurfaces(List<ContactPair> pairs, int contactType,
    ...
    for (int i = nContacts; --i >= 0;) {
      ...
      if (i > 0 && (i % 1000) == 0 && logLevel == 4) {
        ...
        Utilities.getIO().getOut().println("contact..." + i);
        Logger.setLogLevel(0);
      }
      if (colorByType)
  ...
  private List<ContactPair> getPairs(BitSet bsA, BitSet bsB, RadiusData rd,
    ...
    if (Logger.debugging)
      for (int i = 0; i < list.size(); i++)
        Utilities.getIO().getOut().println(list.get(i).toString());
    Utilities.getIO().getOut().println("Contact pairs: " + list.size());
    return list;
  }
