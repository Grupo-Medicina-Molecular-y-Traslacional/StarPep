...
public class Isosurface extends MeshCollection implements MeshDataServer {
  ...
  boolean allowMesh = true;
	...
	public void setProperty(String propertyName, Object value, BitSet bs) {
		...
		// + (propertyName == "token" ? " " +
		// Token.nameOf(((Integer)value).intValue()) : ""));
		...
		if ("atomcolor" == propertyName) {
			if (thisMesh != null) {
				if (thisMesh.vertexSource == null) {
					...
					setProperty("property", new float[viewer.getAtomCount()],
							null);
					if (colix != 0) {
						thisMesh.colorCommand = "color isosurface "
								+ Graphics3D.getHexCode(colix);
						setProperty("color", new Integer(Graphics3D
								.getArgb(colix)), null);
					}
				}
			...
			for (int i = meshCount; --i >= 0;)
				if (meshes[i].connections != null
						&& meshes[i].modelIndex == ((Integer) ((Object[]) value)[0])
								.intValue())
					meshes[i].updateCoordinates(
							(Matrix4f) ((Object[]) value)[2],
		...
		if ("lcaoCartoon" == propertyName || "lonePair" == propertyName
			...
			// z x center rotationAxis (only one of x, y, or z is nonzero; in
			// radians)
		...
		if ("finalize" == propertyName) {
			if (thisMesh != null) {
				...
				if (cmd != null && !cmd.startsWith("; isosurface map")) {
					thisMesh.setDiscreteColixes(
							sg.getParams().contoursDiscrete,
		...
		if ("mapColor" == propertyName || "readFile" == propertyName) {
			if (value == null) {
				// ScriptEvaluator has passed the filename to us as the value of
				// the
				// "fileName" property. We retrieve that from the
				// surfaceGenerator
				// and open a BufferedReader for it. Or not. But that would be
				// unlikely since we have just checked it in ScriptEvaluator
				value = viewer.getBufferedReaderOrErrorMessageFromName(sg
						.getFileName(), null, true);
				if (value instanceof String) {
					Logger.error("Isosurface: could not open file "
							+ sg.getFileName() + " -- " + value);
					return;
				}
				try {
					value = new BufferedReader(new InputStreamReader(
							(InputStream) value, "ISO-8859-1"));
		...
		if ("init" == propertyName) {
			...
			if (script != null
					&& !(iHaveBitSets = getScriptBitSets(script, null)))
		...
		if (propertyName == "deleteModelAtoms") {
			...
			for (int i = meshCount; --i >= 0;) {
				...
				if (m.modelIndex == modelIndex) {
					...
					if (m == currentMesh)
						currentMesh = thisMesh = null;
					meshes = isomeshes = (IsosurfaceMesh[]) ArrayUtil
...
//      thisMesh.bsSlabDisplay = thisMesh.jvxlData.bsSlabDisplay;
  ...
  /*
   ...
   if (!logMessages)
   return;
   Utilities.getIO().getOut().println("Isosurface using testflag2: no associative grouping = "
   + !associateNormals);
   Utilities.getIO().getOut().println("IsosurfaceRenderer using testflag4: show vertex normals = "
   + viewer.getTestFlag4());
   Logger
