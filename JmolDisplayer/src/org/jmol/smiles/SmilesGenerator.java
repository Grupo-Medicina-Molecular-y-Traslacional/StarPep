...
public class SmilesGenerator {
  ...
  private String getRingCache(int i0, int i1, Map<String, Object[]> ht) {
    ...
    if (s == null) {
      ...
      if (Logger.debugging)
        Utilities.getIO().getOut().println("adding for " + i0 + " ring key " + nPairs + ": " + key);
    } else {
      ht.remove(key);
      if (Logger.debugging)
        Utilities.getIO().getOut().println("using ring key " + key);
    }
    return s;//  + " _" + key + "_ \n";
  }
  ...
  private void dumpRingKeys(StringBuffer sb, Map<String, Object[]> ht) {
    ...
    while (e.hasNext()) {
      Utilities.getIO().getOut().println("unmatched ring key: " + e.next());
    }
  }
