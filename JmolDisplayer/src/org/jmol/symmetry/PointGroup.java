...
//                checkAxisOrder(3, v3, center);
  ...
  private class Operation {
    ...
    Operation() {
      ...
      order = 1;
      if (Logger.debugging)
        Utilities.getIO().getOut().println("new operation -- " + typeNames[type]);
    }
    ...
    Operation(Vector3f v, int i) {
      ...
      if (Logger.debugging)
        Utilities.getIO().getOut().println("new operation -- " + (order == i ? "S" : "C") + order + " "
            + normalOrAxis);
    }
    ...
    Operation(Vector3f v) {
      ...
      normalOrAxis = new Quaternion(v, 180).getNormal();
      if (Logger.debugging)
        Utilities.getIO().getOut().println("new operation -- plane " + normalOrAxis);
    }
