/* $RCSfile$
 * $Author: hansonr $
 * $Date: 2011-12-05 06:23:55 +0100 (lun., 05 déc. 2011) $
 * $Revision: 16556 $
 *
...
public class FileManager {
  ...
  void setAppletContext(URL documentBase, URL codeBase, String jmolAppletProxy) {
    ...
    Utilities.getIO().getOut().println("appletDocumentBase=" + appletDocumentBase
  ...
  Object createAtomSeCollectionFromStrings(String[] arrayModels,
    ...
    setLoadState(htParams);
    Utilities.getIO().getOut().println("FileManager.getAtomSetCollectionFromStrings(string[])");
  ...
  Object createAtomSeCollectionFromArrayData(List<Object> arrayData,
    ...
    Utilities.getIO().getOut().println("FileManager.getAtomSetCollectionFromArrayData(Vector)");
    ...
    try {
      if (isApplet || isURL) {
        ...
        if (showMsg && !checkOnly)
          Utilities.getIO().getOut().println("FileManager opening " + url.toString());
        ...
        if (showMsg)
          Utilities.getIO().getOut().println("FileManager opening " + name);
    ...
    if (info != null) {
      ...
      if (info[2] != null) {
        ...
        for (int i = 2; i < info.length; i++) {
          ...
          Utilities.getIO().getOut().println("reading " + name);
  ...
  private static String setScriptFileReferences(String script, String dataPath,
    ...
    for (int iFile = 0; iFile < nFiles; iFile++) {
      ...
      Utilities.getIO().getOut().println("FileManager substituting " + name0 + " --> " + name);
  ...
  private static String writeZipFile(String outFileName,
    ...
    long nBytesOut = 0;
    long nBytes = 0;
    Utilities.getIO().getOut().println("creating zip file " + outFileName + "...");
    ...
    try {
      ...
      for (int i = 0; i < fileNamesAndByteArrays.size(); i += 2) {
        ...
        Utilities.getIO().getOut().println("...adding " + fname);
        String key = ";" + fnameShort + ";";
        if (fileList.indexOf(key) >= 0) {
          Utilities.getIO().getOut().println("duplicate entry");
          continue;
        }
        fileList += key;
      ...
      os.close();
      Utilities.getIO().getOut().println(nBytesOut + " bytes prior to compression");
      if (bos != null) {
      ...
      Utilities.getIO().getOut().println(e.getMessage());
      return e.getMessage();
    }
