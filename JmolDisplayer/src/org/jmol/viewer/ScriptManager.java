...
class ScriptManager {
  ...
  public Object addScript(String returnType, String strScript,
    ...
    //if (Logger.debugging)
    //  Utilities.getIO().getOut().println("ScriptManager queue size=" + scriptQueue.size() + " scripts; added: " 
  ...
  public void waitForQueue() {
    ...
    while (queueThreads[0] != null || queueThreads[1] != null) {
      try {
        ...
        if (((n++) % 10) == 0)
          if (Logger.debugging) {
            Utilities.getIO().getOut().println("...scriptManager waiting for queue: "
  ...
  class ScriptQueueRunnable implements Runnable {
    ...
    private boolean runNextScript() {
      if (scriptQueue.size() == 0)
        return false;
      //Utilities.getIO().getOut().println("SCRIPT QUEUE BUSY" +  scriptQueue.size());
      ...
      if (Logger.debugging) {
        Utilities.getIO().getOut().println("Queue[" + pt + "][" + scriptQueue.size()
      ...
      if (scriptQueue.size() == 0) {// might have been cleared with an exit
        //Utilities.getIO().getOut().println("SCRIPT QUEUE READY", 0);
        return false;
      }
      return true;
    }
  ...
  synchronized void startCommandWatcher(boolean isStart) {
    ...
    if (Logger.debugging) {
      Utilities.getIO().getOut().println("command watcher " + (isStart ? "started" : "stopped")
