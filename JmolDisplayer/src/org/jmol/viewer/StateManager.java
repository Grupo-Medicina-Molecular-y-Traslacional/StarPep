...
public class StateManager {
  ...
  boolean restoreOrientation(String saveName, float timeSeconds, boolean isAll) {
    ...
    //    Utilities.getIO().getOut().println(listSavedStates());
    return true;
  }
    ...
    void unsetUserVariable(String key) {
      key = key.toLowerCase();
        if (key.equals("all") || key.equals("variables")) {
          ...
          Utilities.getIO().getOut().println("all user-defined variables deleted");
        } else if (htUserVariables.containsKey(key)) {
          Utilities.getIO().getOut().println("variable " + key + " deleted");
