/* $RCSfile$
 * $Author: hansonr $
 * $Date: 2011-09-28 23:55:43 +0200 (mer., 28 sept. 2011) $
 * $Revision: 16175 $
 *
...
////////////////////Jmol status //////////////
  ...
  synchronized void setStatusAtomPicked(int atomIndex, String strInfo) {
    ...
    Utilities.getIO().getOut().println("setStatusAtomPicked(" + atomIndex + "," + strInfo + ")");
  ...
  synchronized void setStatusMeasuring(String status, int intInfo, String strMeasure, float value) {
    ...
    if(status.equals("measureCompleted")) { 
      Utilities.getIO().getOut().println("measurement["+intInfo+"] = "+strMeasure);
      sJmol = jmolScriptCallback(EnumCallback.MEASURE);
        ...
        Utilities.getIO().getOut().println("measurePicked " + intInfo + " " + strMeasure);
    }
