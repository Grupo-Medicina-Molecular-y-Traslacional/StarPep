/* $RCSfile$
 * $Author: hansonr $
 * $Date: 2011-12-23 06:17:25 +0100 (ven., 23 déc. 2011) $
 * $Revision: 16631 $
 *
 ...
 *
 *
 * This viewer can also be used with JmolData.jar, which is a frameless version
 * of Jmol that can be used to batch-process scripts from the command line. No
 * shapes, no labels, no export to JPG -- just raw data checking and output.
 *
 *
 *
 * ****************************************************************
...
public class Viewer extends JmolViewer implements AtomDataServer {
    ...
    public SmilesMatcherInterface getSmilesMatcher() {
        if (smilesMatcher == null) {
            smilesMatcher = (SmilesMatcherInterface) Interface.getOptionInterface("smiles.SmilesMatcher");
        }
    ...
    private Viewer(Object display, JmolAdapter modelAdapter,
        ...
        multiTouch = (commandOptions != null && commandOptions.indexOf("-multitouch") >= 0);
        ...
        if (haveDisplay) {
            if (multiTouch) {
                ...
                actionManager = (ActionManager) Interface.getOptionInterface("multitouch.ActionManagerMT");
            } else {
     ...
     * hoverWatcher
     *
     * This is the main access point for creating an application or applet
     * viewer.
     *
    ...
    public synchronized void setAppletContext(String fullName, URL documentBase,
        ...
        if (isApplet) {
            Utilities.getIO().getOut().println("applet context: " + appletContext);
        ...
        /*
         * Utilities.getIO().getOut().println("jvm11orGreater=" + jvm11orGreater + "\njvm12orGreater="
         * + jvm12orGreater + "\njvm14orGreater=" + jvm14orGreater);
         */
        if (!isSilent) {
            Utilities.getIO().getOut().println(JmolConstants.copyright
                    + "\nJmol Version: "
                    + getJmolVersion()
                    + "\njava.vendor: "
                    + strJavaVendor
    ...
    public void translate(char xyz, float x, char type, BitSet bsAtoms) {
        int xy = (type == '\0' ? (int) x : type == '%' ? transformManager.percentToPixels(xyz, x) : transformManager.angstromsToPixels(x
                * (type == 'n' ? 10f : 1f)));
        if (bsAtoms != null) {
            if (xy == 0) {
                return;
    ...
    public void setElementArgb(int elementNumber, int argb) {
        ...
        global.setParameterValue("=color "
                + Elements.elementNameFromNumber(elementNumber), Escape.escapeColor(argb));
    ...
    public Point3f getColorPointForPropertyValue(float val) {
        ...
        return Graphics3D.colorPointFromInt2(g3d.getColorArgbOrGray(colorManager.getColixForPropertyValue(val)));
    }
    ...
    /*
     * public void addLoadScript(String script) { System.out.println("VIEWER
     * addLoadSCript " + script); // fileManager.addLoadScript(script); }
     */
     ...
     * by JmolFileDropper and JmolPanel file-open actions - sets up a script to
     * load the file
     *
     * @param fileName
     */
    ...
    public void openFileAsynchronously(String fileName) {
        ...
        if (fileName.endsWith("jvxl")) {
            ...
            if (type == null) {
                type = SurfaceFileTyper.determineSurfaceFileType(getBufferedInputStream(fileName));
                if (type != null) {
    ...
    public String loadModelFromFile(String fullPathName, String fileName,
        ...
        if (fileNames != null) {
            ...
            Utilities.getIO().getOut().println("openFiles(" + fileNames.length + ") " + ms + " ms");
            ...
            for (int i = 0; i < fileNames.length; i++) {
                ...
                s = TextFormat.simpleReplace(s, "$FILENAME" + (i + 1) + "$", Escape.escape(fname.replace('\\', '/')));
            }
        ...
        if (loadScript != null) {
            ...
            htParams.put("loadScript", loadScript = new StringBuffer(TextFormat.simpleReplace(loadScript.toString(), "$FILENAME$", Escape.escape(fname.replace('\\', '/')))));
        }
     ...
     *
     * @param id if null, clear "bad" entries from the set.
     ...
     * @param isAppend
     * @param htParams
     * @param loadScript only necessary for string reading
    ...
    private Object getAtomSetCollection(String fileName, boolean isAppend,
        ...
        boolean isString = (fileName.equalsIgnoreCase("string") || fileName.equals(JmolConstants.MODELKIT_ZAP_TITLE));
    ...
    public String loadInline(String strModel, char newLine, boolean isAppend, Map<String, Object> htParams) {
        ...
        if (newLine != 0) {
            Utilities.getIO().getOut().println("loading model inline, " + strModel.length()
     ...
     * @param isAppend
     * @param loadScript if null, then some special method like DOM; turn of
     * preserveState
     * @return errMsg
     */
    ...
    public String getModelNumberDotted(int modelIndex) {
        return modelIndex < 0 ? "all" : modelSet == null ? null : modelSet.getModelNumberDotted(modelIndex);
    }
    ...
    public int findNearestAtomIndex(int x, int y, boolean mustBeMovable) {
        return (modelSet == null || !getAtomPicking() ? -1 : modelSet.findNearestAtomIndex(x, y, mustBeMovable ? selectionManager.getMotionFixedAtoms() : null));
    }
     ...
     * 2) Viewer.repaint() invokes display.repaint(), provided display is not
     * null (headless) 3) The system responds with an invocation of
     * Jmol.update(Graphics g), which we are routing through Jmol.paint(Graphics
     * g). 4) Jmol.update invokes Viewer.setScreenDimension(size), which makes
     * the necessary changes in parameters for any new window size. 5)
     * Jmol.update invokes Viewer.renderScreenImage(g, size, rectClip) 6)
    ...
    public void renderScreenImage(Object g, int width, int height) {
         ...
         * viewer.repaint() --> display.repaint() --> OS event queue |
         * Jmol.paint() <-- viewer.renderScreenImage() <--
         * viewer.notifyViewerRepaintDone() <-- repaintManager.repaintDone()<--
    ...
    public boolean checkHalt(String str, boolean isInterrupt) {
        ...
        Utilities.getIO().getOut().println(isCmdLine_c_or_C_Option ? haltType
    ...
    synchronized Object evalStringWaitStatus(String returnType, String strScript,
        ...
        if (isCmdLine_c_or_C_Option) {
            Utilities.getIO().getOut().println("--checking script:\n" + eval.getScript() + "\n----\n");
        }
        ...
        if (isOK) {
            ...
            if (!isQuiet) {
                scriptStatus("Jmol script terminated", strErrorMessage, 1 + eval.getExecutionWalltime(), strErrorMessageUntranslated);
            }
        } else {
        ...
        if (isCmdLine_c_or_C_Option) {
            if (strErrorMessage == null) {
                Utilities.getIO().getOut().println("--script check ok");
            } else {
                Logger.error("--script check error\n" + strErrorMessageUntranslated);
            }
        }
        if (isCmdLine_c_or_C_Option) {
            Utilities.getIO().getOut().println("(use 'exit' to stop checking)");
        }
        isScriptQueued = true;
    ...
    private BitSet addHydrogensInline(BitSet bsAtoms, List<Atom> vConnections,
        ...
        sb.append(pts.length).append("\n").append(JmolConstants.ADD_HYDROGEN_TITLE).append("#noautobond").append("\n");
        for (int i = 0; i < pts.length; i++) {
            sb.append("H ").append(pts[i].x).append(" ").append(pts[i].y).append(" ").append(pts[i].z).append(" - - - - ").append(++atomno).append('\n');
        }
    ...
    void hoverOn(int atomIndex, int action) {
        ...
        if (isBound(action, ActionManager.ACTION_dragLabel)
                ...
                && modelSet.atoms[atomIndex].isShapeVisible(JmolConstants.getShapeVisibilityFlag(JmolConstants.SHAPE_LABELS))) {
    ...
    void setPickingMode(String strMode, int pickingMode) {
        ...
        global.setParameterValue("picking", ActionManager.getPickingModeName(actionManager.getAtomPickingMode()));
    ...
    void setPickingStyle(String style, int pickingStyle) {
        ...
        global.setParameterValue("pickingStyle", ActionManager.getPickingStyleName(actionManager.getPickingStyle()));
    }
    ...
    public String getAtomInfo(int atomOrPointIndex) {
        ...
        return (atomOrPointIndex >= 0 ? modelSet.getAtomInfo(atomOrPointIndex, null) : (String) shapeManager.getShapeProperty(JmolConstants.SHAPE_MEASURES, "pointInfo",
                -atomOrPointIndex));
    }
    ...
    public void setMenu(String fileOrText, boolean isFile) {
        if (isFile) {
            Utilities.getIO().getOut().println("Setting menu "
    ...
    /*
     *
     * animFrameCallback echoCallback (defaults to messageCallback)
     * errorCallback evalCallback hoverCallback loadStructCallback
     * measureCallback (defaults to messageCallback) messageCallback (no local
     * version) minimizationCallback pickCallback resizeCallback scriptCallback
     * (defaults to messageCallback) syncCallback
     */

    /*
     * aniframeCallback is called:
     *
     * -- each time a frame is changed -- whenever the animation state is
     * changed -- whenever the visible frame range is changed
     *
     * jmolSetCallback("animFrameCallback", "myAnimFrameCallback") function
     * myAnimFrameCallback(frameNo, fileNo, modelNo, firstNo, lastNo) {}
     *
     * frameNo == the current frame in fileNo == the current file number,
     * starting at 1 modelNo == the current model number in the current file,
     * starting at 1 firstNo == flag1 * (the first frame of the set, in file *
     * 1000000 + model notation) lastNo == flag2 * (the last frame of the set,
     * in file * 1000000 + model notation)
     *
    ...
    /*
     * echoCallback is one of the two main status reporting mechanisms. Along
     * with scriptCallback, it outputs to the console. Unlike scriptCallback, it
     * does not output to the status bar of the application or applet. If
     ...
     * message Eval.showString calculate, cd, dataFrame, echo, error,
     * getProperty, history, isosurface, listIsosurface, pointGroup, print, set,
     * show, write ForceField.steepestDescentInitialize for initial energy
    ...
    public void scriptEcho(String strEcho) {
        ...
        if (listCommands && strEcho != null && strEcho.indexOf("$[") == 0) {
            Utilities.getIO().getOut().println(strEcho);
        }
    }
    ...
    /*
     * evalCallback is a special callback that evaluates expressions in
     * JavaScript rather than in Jmol.
     *
     ...
     * {}
     *
     * ptLoad == JmolConstants.FILE_STATUS_NOT_LOADED == -1 ptLoad ==
     * JmolConstants.FILE_STATUS_ZAPPED == 0 ptLoad ==
     * JmolConstants.FILE_STATUS_CREATING_MODELSET == 2 ptLoad ==
     * JmolConstants.FILE_STATUS_MODELSET_CREATED == 3 ptLoad ==
     * JmolConstants.FILE_STATUS_MODELS_DELETED == 5
     *
     ...
     * completed -- this call comes when the user hesitates with the mouse over
     * an atom and the "rubber band" is showing
     *
     * jmolSetCallback("measureCallback", "myMeasureCallback") function
     * myMeasureCallback(strMeasure, intInfo, status) {}
     *
     * intInfo == (see below) status == "measurePicked" (intInfo == the number
     * of atoms in the measurement) "measureComplete" (intInfo == the current
     * number measurements) "measureDeleted" (intInfo == the index of the
     * measurement deleted or -1 for all) "measurePending" (intInfo == number of
     * atoms picked so far)
     *
     * strMeasure:
     *
     * For "set picking MEASURE ..." each time the user clicks an atom, a
     * message is sent to the pickCallback function (see below), and if the
     * picking is set to measure distance, angle, or torsion, then after the
     * requisite number of atoms is picked and the pick callback message is
     * sent, a call is also made to measureCallback with a string that indicates
     ...
     * function myMinimizationCallback(app, minStatus, minSteps, minEnergy,
     * minEnergyDiff) {}
     *
     * minStatus is one of "starting", "calculate", "running", "failed", or
     * "done"
     *
     ...
     *
     * atom: a string determinied by the PICKLABEL parameter, which if ""
     * delivers the atom identity along with its coordinates
     *
     * bond: ["bond", bondIdentityString (quoted), x, y, z] where the
     * coordinates are of the midpoint of the bond
     *
    ...
    /*
     * scriptCallback is the primary way to monitor script status. In addition,
     * it serves to for passing information to the user over the status line of
     * the browser as well as to the console. Note that console messages are
     * also sent by echoCallback. If messageCallback is enabled but not
     * scriptCallback, these messages go to the messageCallback function
     * instead.
     *
     * jmolSetCallback("scriptCallback", "myScriptCallback") function
     * myScriptCallback(app, status, message, intStatus,
     * errorMessageUntranslated) {}
     *
     * intStatus == -2 script start -- message is the script itself intStatus ==
     * 0 general messages during script execution; translated error message may
     * be present intStatus >= 1 script termination message; translated and
     ...
     * Eval.logDebugScript -- debugScript messages Eval.pause -- script
     * execution paused message Eval.runEval -- "Script completed" message
     * Eval.script -- Chime "script <exiting>" message Eval.scriptStatusOrBuffer
     * -- various messages for Eval.checkContinue (error message) Eval.connect
     * Eval.delete Eval.hbond Eval.load (logMessages message) Eval.message
     * Eval.runEval (error message) Eval.write (error reading file) Eval.zap
     * (error message) FileManager.createAtomSetCollectionFromFile
     * "requesting..." for Chime-like compatibility actionManager.atomPicked
     ...
     * mySyncCallback(app, script, appletName) { ...[modify script here]...
     * return newScript }
     *
    ...
    public void showUrl(String urlString) {
        ...
        Utilities.getIO().getOut().println("showUrl:" + urlString);
        statusManager.showUrl(urlString);
    }

    /**
     * an external applet or app with class that extends
     * org.jmol.jvxl.MeshCreator might execute:
     *
    ...
    private void setStringProperty(String key, int tok, String value) {
        switch (tok) {
            ...
            case Token.quaternionframe:
                // /11.5.39//
                if (value.length() == 2 && value.startsWith("R")) // C, P -- straightness from Ramachandran angles
                {
    ...
    private void setIntProperty(String key, int tok, int value) {
        switch (tok) {
            ...
            case Token.loglevel:
                ...
                Utilities.getIO().getOut().println("logging level set to " + value);
    ...
    private void jmolTest() {
         ...
         * m.add("MODEL 2");m.add( "HETATM 1 H1 UNK 1 2.457 0.000 0.000 1.00
         * 0.00 H " );m.add( "HETATM 2 C1 UNK 1 1.385 0.000 0.000 1.00 0.00 C "
         * );m.add( "HETATM 3 C2 UNK 1 -1.385 -0.000 0.000 1.00 0.00 C " );
         * v.add(new String[] { "MODEL 2", "HETATM 1 H1 UNK 1 2.457 0.000 0.000
         * 1.00 0.00 H " , "HETATM 2 C1 UNK 1 1.385 0.000 0.000 1.00 0.00 C " ,
         * "HETATM 3 C2 UNK 1 -1.385 -0.000 0.000 1.00 0.00 C " , }); v.add(new
         * String[] {"3","testing","C 0 0 0","O 0 1 0","N 0 0 1"} );
         * v.add("3\ntesting\nC 0 0 0\nO 0 1 0\nN 0 0 1\n"); loadInline(v,
         * false);
         */
    }
    ...
    /*
         * ***************************************************************
         * RasMol has the +Y axis pointing down And rotations about the y axis
         * are left-handed setting this flag makes Jmol mimic this behavior
         *
         * All versions of Jmol prior to 11.5.51 incompletely implement this
         * flag. All versions of Jmol between 11.5.51 and 12.2.4 incorrectly
         * implement this flag. Really all it is just a flag to tell Eval to
         * flip the sign of the Z rotation when specified specifically as
         * "rotate/spin Z 30".
         *
    ...
    private void stopAnimationThreads(String fromWhere) {
        ...
        /*
         * try { System.out.println(Thread.currentThread() + " from " +
         * fromWhere + " stopanimatinoThreads -- waiting 1 second");
         * Thread.sleep(1000); } catch (InterruptedException e) { // TODO }
         * System.out.println(Thread.currentThread() + "Viewer
         * stopAnimationThread NOT canceling rendering"); // cancelRendering();
         */
    ...
    public int[] makeConnections(float minDistance, float maxDistance, int order,
        ...
        // eval
        clearModelDependentObjects();
        //removed in 12.2.1 clearAllMeasurements(); // necessary for serialization
        clearMinimization();
            ...
            case 80:
                if (isKiosk) {
                    ...
                    for (int i = 0; i < 4 && appConsole == null; i++) {
                        appConsole = (isApplet ? (JmolAppConsoleInterface) Interface.getOptionInterface("console.AppletConsole")
                                : (JmolAppConsoleInterface) Interface.getApplicationInterface("jmolpanel.AppConsole")).getAppConsole(this);
                        if (appConsole == null) {
                            try {
    ...
    public String getData(String atomExpression, String type) {
        ...
        } else if (type.equalsIgnoreCase("PDB")) // old crude
        {
     ...
     * then this reads a full set of Double[][] in one function call. Otherwise
     * it reads the values using individual function calls, which each return
     * Double.
     *
     * If the functionName begins with "file:" then data are read from a file
     * specified after the colon. The sign of nX is not relevant in that case.
     * The file may contain mixed numeric and non-numeric values; the
     * non-numeric values will be skipped by Parser.parseFloatArray
     *
     *
     *
     *
     ...
     * interest, it's more a property of Eval/Viewer, and the consoles are
     * really just a mechanism for getting user input and sending results, not
     * saving a history of it all. Ultimately I hope to integrate the mouse
     * picking and possibly periodic updates of position into this history to
     * get a full history. We'll see! BH 9/2006
     */
    /**
    ...
    public String createImage(String fileName, String type, Object text_or_bytes,
        ...
        for (int i = bsFrames.nextSetBit(0); i >= 0; i = bsFrames.nextSetBit(i + 1)) {
            ...
            Utilities.getIO().getOut().println(msg);
            info += msg + "\n";
    ...
    private Object createImage(String fileName, String type, Object text_or_bytes,
        ...
        /*
         *
         * org.jmol.export.image.AviCreator does create AVI animations from
         * Jpegs but these aren't read by standard readers, so that's pretty
         * much useless.
         *
         * files must have the designated width and height
         *
         * text_or_bytes: new Object[] { (File[]) files, (String)
         * outputFilename, (int[]) params }
         *
        ...
        try {
            if (fileName == null) {
                ...
                if (fileName == null) {
                    ...
                    err = fileManager.createZipSet(fileName, (String) text_or_bytes, type.equals("ZIPALL"));
                } else {
                    ...
                    if (err == null) {
                        ...
                        if (err instanceof String) // report error status (text_or_bytes == null)
                        {
        ...
        switch (tokens.length) {
            ...
            case 4:
                if (key.equals("rotateXYBy")) {
                    ...
                    rotateSelected(Parser.parseFloat(tokens[2]), Parser.parseFloat(tokens[3]), null);
                }
                break;
            case 5:
            ...
            case 7:
                if (key.equals("centerAt")) {
                    centerAt(Parser.parseInt(tokens[2]), Parser.parseInt(tokens[3]),
                            new Point3f(Parser.parseFloat(tokens[4]), Parser.parseFloat(tokens[5]), Parser.parseFloat(tokens[6])));
                }
        }
        if (disableSend) {
    ...
    /**
     * Simple method to ensure that the image creator (which writes files) was
     * in fact opened by this viewer and not by some manipulation of the applet.
     * When the image creator is used it requires both a viewer object and that
     * viewer's private key. But the private key is private, so it is not
     * possible to create a useable image creator without working through a
     * viewer's own methods. Bob Hanson, 9/20/2009
     *
     * @param privateKey
     ...
     * saving of inlinedata for later LOAD "" commands 4) turns off the saving
     * of changed atom properties 5) does not guarantee accurate state
     * representation 6) disallows generation of the state
     *
    ...
    private String setLogFile(String value) {
        ...
        if (logFilePath == null || value.indexOf("\\") >= 0
            ...
            try {
                path = (isApplet ? logFilePath + value : (new File(logFilePath + value).getAbsolutePath()));
            } catch (Exception e) {
                value = null;
            }
        }
        if (value == null) {
            Utilities.getIO().getOut().println(GT._("Cannot set log file path."));
        } else {
            if (path != null) {
                Utilities.getIO().getOut().println(GT._("Setting log file to {0}", path));
            }
            logFile = path;
        }
        return value;
    ...
    public void minimize(int steps, float crit, BitSet bsSelected,
        ...
        try {
            if (!isSilent) {
                Utilities.getIO().getOut().println("Minimizing " + bsSelected.cardinality() + " atoms");
            }
     ...
     * @param n number of steps to go back/forward; 0 for all; -1 for clear; -2
     * for clear BOTH
     *
     */
    ...
    public String prompt(String label, String data, String[] list,
            boolean asButtons) {
        JmolPromptInterface jpi = (JmolPromptInterface) Interface.getOptionInterface("console.JmolPrompt");
        return (isKiosk || jpi == null ? "null" : jpi.prompt(label, data, list, asButtons));
    }
