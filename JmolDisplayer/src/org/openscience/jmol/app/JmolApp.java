...
public class JmolApp {
  ...
  private void checkOptions(CommandLine line, Options options) {
    if (line.hasOption("h")) {
      ...
      System.out.println("  Utilities.getIO().getOut().println=[true|false] (true)");
  ...
  public void startViewer(JmolViewer viewer, SplashInterface splash) {  
    ...
    if (script1 != null && script1.length() > 0) {
      if (!isSilent)
        Utilities.getIO().getOut().println("Executing script: " + script1);
      if (splash != null)
    ...
    if (scriptFilename != null) {
      if (!isSilent)
        Utilities.getIO().getOut().println("Executing script from file: " + scriptFilename);
      if (splash != null)
    ...
    if (script2 != null && script2.length() > 0) {
      if (!isSilent)
        Utilities.getIO().getOut().println("Executing script: " + script2);
      if (splash != null)
